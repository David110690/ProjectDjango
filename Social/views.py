...
def create_profile(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Perfil {username} creado')
            return redirect('feed')
    else:
        form = ProfileForm()
        context = {'form':form}
    return render(request, 'social/create_profile.html', context)
...
def follow(request, username):
    ...
    rel.save()
    messages.success(request, f'Estas siguiendo a {username}')
    return redirect('feed')
